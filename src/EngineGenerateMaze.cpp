...
void Engine::GenerateMaze(int lvl) {
    if (lvl == 0) {
        maze.clear();
        coins.clear();

        // Store the data for the generated maze here
        char generated[21][21];  // Generate a 21 by 21 maze
        ...
        while (!path.empty()) {
            ...
            if (cell.first < 0 || cell.first > 20 || cell.second < 0 || cell.second > 20)
                continue;
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (cell2.first >= 0 && cell2.first <= 20 && cell2.second >= 0 && cell2.second <= 20) {
            ...
            if (frontier.empty()) {
                ...
                while (!path.empty() && !found) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        // Make sure that the cell we are going to is within bounds
                        if (c2.first >= 0 && c2.first <= 20 && c2.second >= 0 && c2.second <= 20) {
                            // Check to see if that cell has been visited yet
        ...
        generated[19][9] = 'P';  //(19, 9) is where the player starts
        cameraRotation = 0.0f;   // Set camera rotation to be 0 radians
        generated[1][9] = 'E';   //(1, 9) is where the maze ends
        ...
        for (int i = 0; i < 21; i++)
            for (int j = 0; j < 21; j++)
                switch (generated[i][j]) {
                    // Wall
                    case '#':
                        maze.push_back(glm::vec3(wallSize * 2.0f * i - 21.0f * wallSize, 0.5f, wallSize * 2.0f * j - 21.0f * wallSize));
                        break;
                    // Coin
                    case '*':
                        coins.push_back(glm::vec3(wallSize * 2.0f * i - 21.0f * wallSize, 2.0f, wallSize * 2.0f * j - 21.0f * wallSize));
                        break;
                    // Exit
                    case 'E':
                        exitPos = glm::vec3(wallSize * 2.0f * i - 21.0f * wallSize, 2.0f, wallSize * 2.0f * j - 21.0f * wallSize);
                        break;
                    // Player start position
                    case 'P':
                        cameraPosition = glm::vec3(wallSize * 2.0f * i - 21.0f * wallSize, 2.5f, wallSize * 2.0f * j - 21.0f * wallSize);
                        break;
                }
    } else {
        maze.clear();
        ...
        generated[29][15] = 'P';  //(29, 15) is where the player starts
        cameraRotation = 0.0f;    // Set camera rotation to be 0 radians
            ...
            for (int j = 0; j < 31; j++)
                switch (generated[i][j]) {
                    ...
                    case '#':
                        ...
                        maze.push_back(glm::vec3(wallSize * 2.0f * i - 31.0f * wallSize, 1.5f, wallSize * 2.0f * j - 31.0f * wallSize));
                        break;
                    // Coin
                    case '*':
                    ...
                    // Exit
                    case 'E':
                        exitPos = glm::vec3(wallSize * 2.0f * i - 31.0f * wallSize, 4.0f, wallSize * 2.0f * j - 31.0f * wallSize);
                        break;
                    // Player start position
                    case 'P':
